...
/**
 * Board GUI for implementation with various games Author: Kirill Levin, Troy
 * Vasiga, Chris Ingram Modified by Mr. Benum to handle Key presses on Jan. 3,
 * 2011
 */

// =============================MUST IMPLEMENT KEYLISTENER ====================
public class Board extends JPanel implements KeyListener, IObservable {
	...
	// Grid colours
	private static final Color GRID_COLOR_A = new Color(0, 0, 40);
	private static final Color GRID_COLOR_B = new Color(0, 0, 60);

	// Preset colours for pieces
	private static final Color[] COLOURS = { Color.YELLOW, Color.BLUE, Color.CYAN, Color.GREEN, Color.PINK, Color.WHITE,
			Color.RED, Color.ORANGE };

	// String used to indicate each colour
	private static final String[] COLOUR_NAMES = { "yellow", "blue", "cyan", "green", "pink", "white", "red",
			"orange" };
	...
	public void keyPressed(KeyEvent e) {
		//gets pressed key and notifies tetris class
		key = e.getKeyChar();
		notifyObservers(e);
	}
	...
	/**
	 * 
	 * Adds whatever wants to observe Board class (Tetris) as an Observer
	 */
	public void addObserver(IObserver obs) {
		this.obs = obs;
	}
	/**
	 * Notifies the observer that a key event has occured 
	 */
		...
		for (int i = 0; i < this.grid.length; i++) {
			for (int j = 0; j < this.grid[i].length; j++) {
				...
				if (curColour != null) // Draw pegs if they exist
				{
					...
					//mad pegs square
					g.fillRect(curX+1, curY+1, deltaX-1, deltaY-1);
				}
			}
		}
	...
	/**
	 * Convert a String to the corresponding Color defaulting to Black with an
	 * invald input
	 */
	...
	public void paintComponent(Graphics g) {
		//Fix for JOptionPane appearing in corner of screen
	...
	/**
	 * This method will save the value of the colour of the peg in a specific spot.
	 * theColour is restricted to "yellow", "blue", "cyan", "green", "pink",
	 * "white", "red", "orange" Otherwise the colour black will be used.
	 */
	...
	/**
	 * Removes one line from a board given the co-ordinates as endpoints If there is
	 * no such line, nothing happens If multiple lines, all copies are removed
	 */
	...
	public void removeLine(int row1, int col1, int row2, int col2) {
		...
		while (curLine < this.numLines) {
			...
			if ((line[0][curLine] == col1 && line[1][curLine] == row1 && line[2][curLine] == col2
					&& line[3][curLine] == row2)
					|| (line[2][curLine] == col1 && line[3][curLine] == row1 && line[0][curLine] == col2
							&& line[1][curLine] == row2)) {
